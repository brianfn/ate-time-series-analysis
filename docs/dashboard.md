...
資料工程不只是在資料庫中建立表格，也需要讓使用者理解資料治理成果。
...
> 我使用 Streamlit 將後端 SQL Fact Table 轉換成可互動 Dashboard。

> Dashboard 不只是畫圖，而是將資料治理結果轉成製造現場能理解的語言，例如 False Positive 攔截次數、True Anomaly 告警、潛在漏報稽核與預估避免停機成本。

> 這代表我不只會寫 ETL，也能將資料工程成果產品化，讓製程、MES、研發或管理者能直接使用。
